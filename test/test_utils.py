...
class TestCheckpoint(TestCase):
    ...
    def test_checkpointing_without_reentrant_early_free(self):
        ...
        device_mod = getattr(torch, _acc.type, None)
        if device_mod is None or not hasattr(device_mod, "memory_allocated"):
            self.skipTest(f"{_acc.type} does not support memory_allocated tracking")
